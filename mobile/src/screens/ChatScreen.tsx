  ...
  return (
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={8}>
      <View style={styles.header}>
        ...
        <Pressable onPress={async () => { try { const conversation = await getConversation(conversationId); const callId = await createCall(conversationId, conversation.participants, 'audio', title); navigation.navigate(conversation.type === 'group' ? 'GroupCall' : 'Call', { callId, title, kind: 'audio' }); } catch (error: any) { Alert.alert('Appel', error?.message || "Impossible de démarrer l’appel."); } }} style={styles.headerAction}><Ionicons name="call-outline" size={21} color={colors.text} /></Pressable>
        <Pressable onPress={async () => { try { const conversation = await getConversation(conversationId); const callId = await createCall(conversationId, conversation.participants, 'video', title); navigation.navigate(conversation.type === 'group' ? 'GroupCall' : 'Call', { callId, title, kind: 'video' }); } catch (error: any) { Alert.alert('Appel vidéo', error?.message || "Impossible de démarrer l’appel vidéo."); } }} style={styles.headerAction}><Ionicons name="videocam-outline" size={22} color={colors.text} /></Pressable>
      </View>
